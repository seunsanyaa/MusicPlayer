...
export default function MusicPlayer() {
	...
	const fetchSongDetails = useQuery(['fetchSongDetails', songurl], {
		queryFn: async () => {
			...
			const songTotalDurationAsString = durationOfSongString(audioBuffer);

			setTotalDuration(songTotalDurationAsString);
	...
	const intervalHandler = () => {
		...
		if (
			audioBufferRef.current.duration === audioContext?.currentTime ||
			audioBufferRef.current.duration < audioContext?.currentTime
		) {
	...
	const durationOfSongString = (audioBuffer) => {
		if (audioBuffer) {
	...
	useEffect(() => {
		if (intervalRef.current != 0) {
			const fixedPercent = Math.round(
				(Math.floor(audioContextRef.current.currentTime) /
